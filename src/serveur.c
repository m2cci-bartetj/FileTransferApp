...
void serveur_appli(char *service)
{
	...
	while (1)
	{
		/* 2. ACCEPTATION D'UNE CONNEXION - CREATION D'UN PROCESSUS POUR CE CLIENT*/
		id_new_socket = h_accept(id_socket, &p_adr_client);
		if (id_new_socket < 0)
		{
			perror("Erreur accept");
			continue; // on attend une nouvelle connexion
		}

		// Création d'un processus pour ce client
		pid_t pid = fork();
		...
		if (pid == 0)
		{
			...
			// Fermeture de la socket d'écoute pas utile pour le processus enfant
			h_close(id_socket);

			printf("Nouveau client pris en charge dans le process %d\n", getpid());

			// Variables propres à ce processus
			...
			while (choice != '3')
			{
				...
				case '2':
					...
					// /!\ Créer un bug suite à l'implementation du fork
			...
			// Le parent n'utilise pas la socket de donnée, il retourne à accept()
            h_close(id_new_socket);
		} 
	}
}
